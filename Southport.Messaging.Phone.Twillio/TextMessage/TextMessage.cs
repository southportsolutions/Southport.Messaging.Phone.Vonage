...
public class TextMessage : VonageClientBase, ITextMessage
{

    public TextMessage(HttpClient httpClient, IVonageOptions options) : base(options)
    {
        //MessageServiceSid = options.MessagingServiceSid;
    }

    public TextMessage(string apiKey, string secret, bool useSandbox = false, string testPhoneNumbers = null) : base(apiKey, secret, useSandbox, testPhoneNumbers)
    {
    }
    ...
    public async Task<ITextMessageResponse> SendAsync()
    {
        ...
        try
        {
            ...
            return (TextMessageResponse) response;
        }
        catch (VonageSmsResponseException e) // todo correct exception
        {
            ...
            return TextMessageResponse.Failed(e.Message, e.Response.Messages.FirstOrDefault()?.StatusCode.ToString(), statusCode);
        }
    ...
    private async Task<ITextMessageResponse> SendTestPhoneNumbersAsync(string from)
    {
        ...
        {
            messageResponse = (TextMessageResponse)await InnerClient.SmsClient.SendAnSmsAsync(new SendSmsRequest
            {
                To = To,
                From = From,
                Text = Message
